...
def train_model(model, dataloaders, criterion, optimizer, config):
    ...
    for epoch in range(config.epochs):
        ...
        for phase in ['train']:
            ...
            with tqdm(total=lent * config.batchSize) as pbar:
        ...
        if (epoch + 1) % test_epoch == 0:
            with torch.no_grad():
                ...
                print("train_MRE(SD): %f(%f), SDR([1mm, 2mm, 2.5mm, 3mm, 4mm]): " % (
                    torch.mean(train_MRE).detach().cpu().numpy(),
...
def val(model, dataloaders, criterion, optimizer, config):
    ...
    if best_MRE > torch.mean(test_MRE).detach().cpu().numpy():
        ...
        torch.save(model.state_dict(), os.path.join('output', 'best.pkl'))

    # torch.save(model, os.path.join('output', str(epoch) + saveName+ '.pkl'))
